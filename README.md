...
## 🔓 비밀번호 없이 여는 갤러리 / 활동 보고

### 반 갤러리 — `config.js`의 `openClasses`
스프레드시트 **“클래스”** 탭 C열(비번)에 값이 남아 있어도, `openClasses`에 적어 둔 반은
브라우저에서 **비밀번호 화면을 띄우지 않습니다**(작품·그림책이 바로 보여요).

```js
openClasses: ["백령종합사회복지관", "옹진가족센터", "영흥지역아동센터"]
```

- 반 이름 · 반 코드 · 사진폴더명(기관 이름) 중 **아무거나** 적으면 됩니다.
- 🎨 작품 · 📚 그림책 · 📷 **수업 모습**까지 모두 비번 없이 열립니다.
- 학교 카드의 🔒 표시도 함께 사라집니다.
- ⚠️ 링크만 알면 누구나 그 반 작품과 **수업 사진(아이 얼굴)** 을 볼 수 있게 됩니다.
- 시트 C열의 비번 칸을 아예 비우면 서버에서도 공개가 되므로 더 깔끔합니다.

수업 사진 웹앱은 비번 대조를 Apps Script에서만 하므로, 공개 반에서는 **페이지가 열람 키를
대신 넣어 줍니다**. 기본값은 `adminPassword`이고, 따로 두고 싶으면 `openPhotoPw`에 적으세요.

```js
openPhotoPw: "",   // 비워 두면 adminPassword 를 그대로 씀
```

### 활동 보고 — `report.html`의 `OPEN_PW`
열람용 비밀번호를 **페이지가 대신 넣어 줍니다**. 방문자는 비번 화면을 보지 않습니다.

```js
var OPEN_PW = "181818";   // report.html 위쪽
```

- Apps Script 쪽 열람 비번을 바꾸면 **이 한 줄만** 같이 바꾸면 됩니다.
- 이 키가 거부될 때만 예비 비밀번호 칸이 나옵니다(예전 화면 그대로).
